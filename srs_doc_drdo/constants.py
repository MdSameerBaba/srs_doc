...
SRS_SECTIONS = {
    ...
    "4_product_description": {
        ...
        "subsections": {
            ...
            "4_1_2_command_transmission":"4.1.2 Command Transmission",
             "4_1_3_data_logging":        "4.1.3 Data Logging",
    ...
    "5_system_features": {
        ...
        "subsections": {
            "5_1_core_features":    "5.1 Core System Features",
            "5_2_data_handling":    "5.2 Data Handling and Output",
        }
    },
    "6_states_and_modes": {
...
_NO_CODE_RULE = (
    ...
    "- Do NOT use your own general/training knowledge to explain what a term, acronym, or technology 'usually' means "
    "when the project files themselves do not explicitly state it. Recognizing a familiar-looking word or substring "
    "inside an identifier (e.g., seeing \"ISO\" inside \"isoformat\", or \"CLI\" inside a folder name) is NOT evidence "
    "of an explicit definition in this project — only quote/paraphrase a meaning if the project files state it themselves.\n"
...
Identify each distinct, major feature or capability actually implemented in the project (for example: a specific user-facing operation, a calculation routine, a CRUD operation, a processing pipeline, or a UI screen). Use the project's own terminology for feature names — do not invent feature names from a generic template. For each feature found, describe its Description, Inputs, Processing, Outputs, and Source File(s) as defined above.

**5.2 Data Handling and Output**
Describe how the system records, stores, persists, or outputs data as evidenced by the project files (for example: console output, file writes, database writes, logging, or returned API responses). State what is recorded, when, in what format, and reference the file names that provide evidence. If no such capability exists, state: "Not determinable from the provided codebase."

Do NOT use any fixed list of feature names from outside this project. Every feature subsection title and count must come from what is actually present in the uploaded files — a project may have one feature or twenty; do not force the document into a predetermined number of features.
...
**this section shouldnt generate anything new it should reflect on the content that hs been generated before this in the sections 7,8,9,10 .**
**11.1 Requirements to Source File Traceability**
| REQ-ID | Requirement Summary | Section | Source File(s) |
List ALL requirement IDs identified across Sections 7–10, one row per requirement and use only the same REQ-ID and Requirement Summary content generated used in sections 7, 8 , 9,10 STRICTLY DO NOT GENERATE ANYTHING NEW HERE JUST USE THE BEFORE CONTENT AND THERE REQ-ID DONT USE ANYTHING NEW.
...
use only the same REQ-ID and Requirement Summary and System Feature / Module and Verification Method content generated used in sections 7, 8 , 9,10.
STRICTLY DO NOT GENERATE ANYTHING NEW HERE JUST USE THE BEFORE CONTENT AND THERE REQ-ID DONT USE ANYTHING NEW
**11.3 Traceability Summary**
**generate the count values according to the content generated before do not add any new content **
