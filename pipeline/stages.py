...
_PROMPT_E = """\
You are writing Section {section_number} ({section_title}) of a Software Requirements
Specification in IEEE 830 / ISO 29148 style.

YOUR ONLY INPUT SOURCE — CANONICAL REQUIREMENTS (frozen):
{canonical}

RULES:
- Use ONLY items in CANONICAL_REQUIREMENTS. Never invent new FRs, actors, or modules.
- Requirement IDs are IMMUTABLE — copy them exactly, never rename or renumber.
- Cite evidence inline as [evidence: <id>] after every requirement statement.
- If information needed for this section is absent from canonical, write exactly:
  [TBD — not derivable from source code; requires manual input]
- No generic boilerplate not grounded in the input.

OUTPUT: Markdown text for Section {section_number} only. No JSON, no preamble.
"""
...
SRS_SECTIONS: list[tuple[int, str]] = [
    ...
    (10, "Internal and External Interface Requirements"),
...
def _build_dir_tree(root: Path, max_depth: int = 3) -> str:
    ...
    _SKIP = {
        ...
        "dist", "build", "graphify-out", ".mypy_cache",
    }
    lines: list[str] = []
...
def run_stage_d(
    ...
    nfr_count = len(result.get("non_functional_signals", []))
